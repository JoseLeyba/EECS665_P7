#ifndef LEVIATHAN_AST_HPP
#define LEVIATHAN_AST_HPP
...
public:
	InitializerNode(const Position * p, std::list<ExpNode *> * nodesIn, bool isArrayIn)
	: ASTNode(p), myNodes(nodesIn), isArray(isArrayIn){}
	void unparse(std::ostream& out, int indent) override;
	bool nameAnalysis(SymbolTable * symTab) override;
	virtual void typeAnalysis(TypeAnalysis *);
	virtual Opd * flatten(IRProgram * prog);
	virtual Opd * flatten(Procedure * prog);
private:
	std::list<ExpNode *> * myNodes;
	bool isArray;
...
public:
	...
	TypeNode * inType, InitializerNode * inInit)
...
public:
	...
	InitializerNode * initIn)
	: VarDeclNode(p, id, type, initIn){ }
	void unparse(std::ostream& out, int indent) override;
	virtual void to3AC(Procedure * proc) override;
	virtual void to3AC(IRProgram * prog) override;
};
...
public:
	ReadStmtNode(const Position * p, LocNode * inSrc, LocNode * inDst)
	: StmtNode(p), mySrc(inSrc), myDst(inDst){ }
	void unparse(std::ostream& out, int indent) override;
	bool nameAnalysis(SymbolTable * symTab) override;
	virtual void typeAnalysis(TypeAnalysis *) override;
	virtual void to3AC(Procedure * prog) override;
...
public:
	WriteStmtNode(const Position * p, ExpNode * inDst, ExpNode * inSrc)
	: StmtNode(p), myDst(inDst), mySrc(inSrc){ }
	void unparse(std::ostream& out, int indent) override;
	bool nameAnalysis(SymbolTable * symTab) override;
	virtual void typeAnalysis(TypeAnalysis *) override;
...
public:
	SinkStmtNode(const Position * p, IDNode * idIn)
