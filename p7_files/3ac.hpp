...
public:
	WriteQuad(Opd * src, Opd * dst, const DataType * type);
	std::string repr() override;
...
public:
	ReadQuad(Opd* src, Opd* dst, const DataType * dstType);
	std::string repr() override;
	Opd * getSrc(){ return mySrc; }
	Opd * getDst(){ return myDst; }
