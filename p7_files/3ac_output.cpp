...
void WriteStmtNode::to3AC(Procedure * proc){
	if (mySrc == nullptr){ throw new InternalError("write mySrc null!"); }
	if (myDst == nullptr){ throw new InternalError("write myDst null!"); }

	Opd * srcOpd = mySrc->flatten(proc);
	Opd * dstOpd = myDst->flatten(proc);
	if (srcOpd == nullptr){ throw new InternalError("write srcOpd null!"); }
	if (dstOpd == nullptr){ throw new InternalError("write dstOpd null!"); }
...
void SinkStmtNode::to3AC(Procedure * proc){
	//Does nothing
}
...
void VarDeclNode::to3AC(Procedure * proc){
	...
	if (myInit != nullptr){
		auto symOpd = proc->getSymOpd(sym);
		auto i = myInit->flatten(proc);	
		Quad * asgInit = new AssignQuad(symOpd, i);
		proc->addQuad(asgInit);
	}
}
...
void VarDeclNode::to3AC(IRProgram * prog){
	...
	if (myInit != nullptr){
		Opd * symOpd = prog->getGlobal(sym);
		auto i = myInit->flatten(prog);
		Quad * asgInit = new AssignQuad(symOpd, i);
		Procedure * initProc = prog->getInitProc();
		initProc->addQuad(asgInit);
	}
}

Opd * InitializerNode::flatten(Procedure * proc){
	auto exp = myNodes->front();
	auto i = exp->flatten(proc);	
	return i;
}

Opd * InitializerNode::flatten(IRProgram * prog){
	Procedure * initProc = prog->getInitProc();
	auto exp = myNodes->front();
	auto i = exp->flatten(initProc);	
	return i;
}
