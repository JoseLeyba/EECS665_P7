...
WriteQuad::WriteQuad(Opd * src, Opd * dst, const DataType * type)
: mySrc(src), myDst(dst), mySrcType(type){ }

std::string WriteQuad::repr(){
	return "WRITE " + mySrc->valString() + ", " + myDst->valString();
}

ReadQuad::ReadQuad(Opd * src, Opd * dst, const DataType * type)
: mySrc(src), myDst(dst), myDstType(type){ }

std::string leviathan::ReadQuad::repr(){
	return "READ " + mySrc->valString() + ", " + myDst->valString();
}
