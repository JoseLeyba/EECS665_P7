...
using ArgsType = std::list<const leviathan::DataType *>;
...
public:
	...
	: myName(nameIn), myType(typeIn), sunk(false){ 
	}
...
public:
	SinkSymbol(SemSymbol * sinkee)
	: SemSymbol(sinkee->getName(), nullptr), mySinkee(sinkee){ 
		sunk = true;
	}
	virtual SymbolKind getKind() const override { return SINK; }
	virtual void surface() override { this->mySinkee->surface(); }
	SymbolKind getKind(){ return SINK; }
private:
	SemSymbol * mySinkee;
};
...
class ScopeTable {
	...
	private:
		std::list<std::pair<std::string, SemSymbol *>> * symbols;
};
